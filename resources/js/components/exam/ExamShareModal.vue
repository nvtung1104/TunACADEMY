<template>
  <AppModal v-model="show" title="Chia sẻ đề thi" size="sm">
    <div class="space-y-4">
      <div>
    ...
    <template #footer>
      ...
      <button @click="doShare" :disabled="sharing"
        class="px-4 py-2 rounded-xl bg-[#d63015] text-white text-sm hover:bg-[#c02a10] disabled:opacity-60 font-medium">
...
const visibilityOptions = [
  { value: 'private',  label: 'Riêng tư',   desc: 'Để ở trạng thái riêng tư' },
...
watch(() => props.modelValue, (open) => {
  ...
  Object.assign(shareForm, {
    visibility:   props.exam.visibility ?? 'class',
    classroom_id: props.exam.classroom?.id ?? '',
    student_codes: '',
  })
  shareError.value = ''
})
...
async function doShare() {
  ...
  try {
    ...
    await api.post(`/teacher/exams/${props.exam.id}/share`, payload)
    show.value = false
    emit('shared')
  } catch (e) {
    shareError.value = e.response?.data?.message ?? 'Có lỗi xảy ra'
  } finally {
    sharing.value = false
  }
}

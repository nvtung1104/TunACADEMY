  ...
  <div>
    ...
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1 min-w-0">
        <div v-if="exam" class="flex flex-wrap items-center gap-2 mb-1.5">
          <span class="inline-flex px-2.5 py-0.5 rounded-lg text-xs font-semibold" :class="typeClass(exam.type)">
            {{ typeLabel(exam.type) }}
          </span>
        ...
        <h2 class="text-xl font-bold text-gray-900 truncate leading-tight">{{ exam?.title ?? 'Đang tải...' }}</h2>
        <p class="text-sm text-gray-400 mt-1 flex items-center gap-1.5 flex-wrap">
          <span>📚 {{ exam?.subject?.name }}</span>
          <span class="text-gray-300">·</span>
          <span>🏫 {{ exam?.classroom?.name ?? 'Công khai' }}</span>
          <span v-if="exam?.duration_minutes" class="text-gray-300">·</span>
          <span v-if="exam?.duration_minutes">⏱️ {{ exam.duration_minutes }} phút</span>
        </p>
      </div>
      <!-- Stats pills -->
      <div v-if="exam" class="hidden md:flex items-center gap-3 shrink-0">
        ...
        <div class="text-center px-4 py-2 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <p class="text-xl font-black text-[#d63015]">{{ totalPoints }}</p>
          <p class="text-xs text-gray-400 font-semibold mt-0.5">Tổng điểm</p>
        </div>
        <div class="text-center px-4 py-2 bg-white rounded-2xl border border-gray-100 shadow-sm">
    ...
    <div v-if="activeTab === 'questions'" class="space-y-4">
      ...
      <div v-else-if="!questions.length"
        ...
        <h4 class="font-bold text-gray-700">Chưa có câu hỏi nào trong đề thi này</h4>
        <p class="text-sm text-gray-400 mt-1 max-w-sm mx-auto leading-relaxed">
          Đề thi của bạn hiện đang trống. Hãy tạo một câu hỏi mới hoặc lấy các câu hỏi có sẵn từ Ngân hàng câu hỏi của bạn.
        </p>
        ...
        <div v-for="(q, idx) in questions" :key="q.id"
          ...
          <div class="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button @click="confirmDelete(q)"
              class="p-2 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-600 transition-all" title="Xóa khỏi đề thi">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div v-else-if="activeTab === 'preview'" class="space-y-4">
      <div v-if="!questions.length" class="py-16 text-center text-gray-400 bg-white rounded-3xl border border-gray-100">
        Hãy thêm ít nhất một câu hỏi vào đề thi để có thể sử dụng tính năng xem trước.
      </div>
    ...
    <div v-else-if="activeTab === 'results'">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap gap-3 mb-5">
        ...
        <select v-model="filterStatus" @change="fetchAttempts()" class="px-4 py-2 text-sm rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#d63015] bg-white text-gray-600 font-medium">
      ...
      <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div v-if="aLoading" class="py-16 text-center text-gray-400">
        ...
        <table v-else class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th class="text-left px-5 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider hidden md:table-cell">Thời gian làm</th>
              <th class="text-left px-5 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Điểm</th>
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="(a, idx) in attempts" :key="a.id" class="hover:bg-gray-50/50 transition-colors">
              <td class="px-5 py-3 text-gray-400 text-xs font-bold">
                {{ (attemptMeta.current_page - 1) * 30 + idx + 1 }}
              </td>
              <td class="px-5 py-3">
                <p class="font-semibold text-gray-800">{{ a.student?.name ?? '—' }}</p>
                <p class="text-xs text-gray-400">{{ a.student?.email }}</p>
              </td>
              <td class="px-5 py-3 hidden sm:table-cell">
                <p class="text-sm text-gray-700 font-medium">{{ a.submitted_at ? formatDateTime(a.submitted_at) : '—' }}</p>
              </td>
              <td class="px-5 py-3 hidden md:table-cell">
                <span v-if="a.started_at && a.submitted_at" class="text-xs text-gray-600 bg-gray-100 px-2 py-0.5 rounded-lg font-medium">
              ...
              <td class="px-5 py-3">
                <template v-if="a.score != null">
                  <span class="text-base font-black text-green-600">{{ a.score }}</span>
                  <span class="text-xs text-gray-400 font-semibold">/10</span>
                </template>
                <span v-else class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full font-medium">Chưa nộp</span>
              </td>
              <td class="px-5 py-3 hidden lg:table-cell">
                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold"
                  :class="a.status === 'submitted' ? 'bg-green-50 text-green-600 border border-green-100' : a.status === 'in_progress' ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-gray-100 text-gray-500'">
        ...
        <div class="bg-white rounded-3xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col h-[80vh]">
          ...
          <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-400 mt-0.5">Môn học: <strong class="text-[#d63015]">{{ exam?.subject?.name }}</strong></p>
            </div>
          ...
          <div class="flex-1 overflow-y-auto divide-y divide-gray-50">
            ...
            <div v-else-if="bankModal.questions.length === 0" class="py-16 text-center text-gray-400 text-sm">
              Không tìm thấy câu hỏi nào có môn học trùng với đề thi này trong ngân hàng.
            </div>
            <template v-else>
              <div v-for="q in bankModal.questions" :key="q.id" class="px-6 py-4 hover:bg-gray-50/50 transition-colors flex items-start gap-4 justify-between">
                ...
                <div class="shrink-0 ml-3">
                  <button v-if="isAlreadyAdded(q.id)" @click="toggleBankQuestion(q)"
                    class="p-2 rounded-xl bg-red-50 text-[#d63015] hover:bg-red-100 hover:text-red-700 transition-colors" title="Bỏ gán khỏi đề thi">
                  ...
                  <button v-else @click="toggleBankQuestion(q)"
                    class="p-2 rounded-xl bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-[#d63015] transition-colors" title="Thêm vào đề thi">
    ...
    <AppModal v-model="deleteModal" title="Xác nhận xóa" size="sm">
      <p class="text-sm text-gray-600 leading-relaxed">Bạn có chắc muốn xóa câu hỏi này khỏi đề thi?</p>
      <template #footer>
...
const tabs = [
  ...
  { key: 'results',   label: 'Kết quả nộp', icon: '🏆' },
]
...
function submitPreMock() {
  ...
  questions.value.forEach(q => {
    ...
    if (q.type === 'multiple_choice' || q.type === 'true_false' || q.type === 'listening') {
      ...
      // Mock grade essay/short response if matching standard
...
async function fetchBankQuestions() {
  ...
  try {
    const { data } = await api.get('/teacher/question-bank', {
      params: {
        subject_id: exam.value?.subject_id,
...
async function toggleBankQuestion(q) {
  ...
  try {
    if (isAdded) {
      // Remove
      await api.post(`/teacher/exams/${examId}/remove-questions`, { question_ids: [q.id] })
      toast.success('Đã loại bỏ câu hỏi khỏi đề')
    } else {
      // Add
      await api.post(`/teacher/exams/${examId}/import-questions`, { question_ids: [q.id] })
...
async function fetchExam() {
  ...
  try {
    ...
    toast.error(err.response?.data?.message ?? 'Không thể tải thông tin đề thi. Vui lòng thử lại.')
  } finally {
...
async function saveQuestion(payload) {
  ...
  try {
    if (editQuestion.value) {
      await api.put(`/teacher/exams/${examId}/questions/${editQuestion.value.id}`, payload)
...
async function doDelete() {
  ...
  try {
    ...
    if (q.question_bank_id) {
      // Detach imported bank question
      await api.post(`/teacher/exams/${examId}/remove-questions`, { question_ids: [q.question_bank_id] })
    } else {
      // Delete direct question
      await api.delete(`/teacher/exams/${examId}/questions/${q.id}`)
    }
